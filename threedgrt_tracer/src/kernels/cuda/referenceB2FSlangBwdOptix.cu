...
static __device__ __inline__ void trace(
    ...
    optixTrace(params.handle, rayOri, rayDir,
               ...
               OPTIX_RAY_FLAG_DISABLE_CLOSESTHIT | (PipelineParameters::SurfelPrimitive ? OPTIX_RAY_FLAG_NONE : OPTIX_RAY_FLAG_CULL_FRONT_FACING_TRIANGLES),
...
extern "C" __global__ void __raygen__rg() {
    ...
    const float3 bwdRayOrigin    = make_float3(rayOrigin.x + (rayMaxHitDistance - epsT) * rayDirection.x,
                                               rayOrigin.y + (rayMaxHitDistance - epsT) * rayDirection.y,
                                               rayOrigin.z + (rayMaxHitDistance - epsT) * rayDirection.z);
    const float3 bwdRayDirection = -rayDirection;

    const float endT = rayMaxHitDistance + epsT;
    float startT     = 0.0f;
    ...
    while (startT < endT) {
        ...
        trace(rayPayload, bwdRayOrigin, bwdRayDirection, startT + epsT, endT);
