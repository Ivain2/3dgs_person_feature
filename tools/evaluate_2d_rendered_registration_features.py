#!/usr/bin/env python3
"""V3.0.2 Task D: 2D-Render Evaluation of Pure Registered Features.

Uses the same registered Gaussian features as Task C, but routes them through
render_person_feature_map + ROI pooling. This tests whether the renderer/ROI
destroys the discriminative power of registered features.

Parameters:
  --registered_features outputs/v3_0_2_pure_registration/registered_features.pt
  --feature_set beta|purity07
  --pooling topk_opacity --topk_ratio 0.3
"""
...
def generate_final_report(v3_dets, alignment_metrics, retrieval_metrics,
    ...
    r = f"# V3.0.2 Render 2D Evaluation Report ({method_name})\n\n"
    r += f"## Date: {time.strftime('%Y-%m-%d %H:%M')}\n\n"
    r += f"## Configuration\n\n"
    r += f"- feature_set: {feature_set}\n"
    r += f"- method: render_person_feature_map + ROI pooling\n\n"
...
def main():
    ...
    print(f"V3.0.2/3.0.3: Render 2D Evaluation (feature_set={args.feature_set}){lin_str}")
    print("=" * 60)

    print("\n[1/7] Loading registered features...")
    ...
    if args.feature_set == "beta":
        ...
        valid_mask_gauss = reg["mask_beta"]
    else:
        features = reg["features_purity07"]
        valid_mask_gauss = reg["mask_purity07"]
    feat_dim = features.shape[1]
    N = features.shape[0]
    print(f"  Feature shape: {features.shape}")
    print(f"  Valid Gaussians: {valid_mask_gauss.sum()}/{N}")
    ...
    valid_mask_device = valid_mask_gauss.to(device)
    print(f"  Overridden _person_feature: {features.shape}")
    print(f"  Feature norm (mean): {features.norm(dim=1).mean().item():.4f}")

    print("\n[3/7] Loading dataset and V2 data...")
        ...
        for cam_id in ["C1", "C2", "C3", "C4", "C5", "C6", "C7"]:
            ...
            with torch.no_grad():
                pf_map, opacity_map = model.render_person_feature_map(
                    gpu_batch, train=False, frame_id=0, valid_mask=valid_mask_device,
            ...
            for d in dets:
                ...
                if args.pooling == "topk_opacity":
                    feat, _ = roi_pool_topk_opacity(pf_map, opacity_map, bbox_render,
                                                    topk_ratio=args.topk_ratio, detach_opacity_weight=True)
                else:
    ...
    elapsed = time.time() - t0
    print(f"  Extracted {len(v3_dets)} features in {elapsed:.0f}s (zero-ROI: {zero_roi})")
    ...
    generate_final_report(
        ...
        pairwise_metrics, association_metrics, diagnostics,
        args.out_dir, args.feature_set, "render2D")

    print(f"\n{'='*60}")
    print(f"V3.0.2 Render 2D Evaluation Complete")
