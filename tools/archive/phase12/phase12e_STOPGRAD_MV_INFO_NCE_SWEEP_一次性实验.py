#!/usr/bin/env python3
"""
Phase 12E: Gaussian-Set + Stop-Gradient Multi-View InfoNCE

Goal: push cross_view_gap beyond +0.05 while keeping fixed_cos >= 0.95.

Uses stop-gradient InfoNCE: anchor has gradient, target_bank is detached.
"""
...
def run_fixed_eval(args, model, batch_builder, fixed_eval_samples, device):
    """Evaluate fixed eval samples: teacher loss + InfoNCE."""
...
def run_phase12e(args, trainer, batch_builder, allowed_cameras):
    print(f"\n{'='*70}")
    print(f"PHASE 12E: GAUSSIAN-SET + STOP-GRADIENT INFO-NCE")
    print(f"{'='*70}")
    print(f"lambda_mv={args.lambda_mv}, tau_mv={args.tau_mv}, P={args.P}, K={args.K}")
    ...
    for step in range(args.num_steps):
        ...
        if step % args.log_interval == 0:
            log_line = f"[PHASE12E] Step {step:5d}: train_loss={L_total.item():.4f} " \
        ...
        if step % args.eval_interval == 0 or step == args.num_steps - 1:
            with torch.no_grad():
                fixed_eval_result = run_fixed_eval(args, trainer.model, batch_builder, fixed_eval_samples, trainer.device)
    ...
    print(f"\n{'='*70}")
    print(f"PHASE 12E RESULT (lambda_mv={args.lambda_mv})")
    print(f"{'='*70}")
...
def main():
    ...
    parser.add_argument('--output_dir', type=str, default='outputs/phase12e_gaussianset_mv_lam005_tau02')
    ...
    parser.add_argument('--lambda_mv', type=float, default=0.05)
    ...
    run_phase12e(args, trainer, batch_builder, allowed_cameras)
    sys.exit(0)
