...
struct GUTRenderer::GutRenderForwardContext {
    ...
    inline Status updateParticlesWorkingBuffers(int numParticles, cudaStream_t cudaStream, const Logger& logger) {
        ...
        if (!uptodate) {
            ...
            CUDA_CHECK_RETURN(cudaMemsetAsync(particlesTilesCount.data(), 0, numParticles * sizeof(uint32_t), cudaStream), logger);
            CHECK_STATUS_RETURN(particlesTilesOffset.resize(numParticles * sizeof(uint32_t), queueHandle, logger));
            CUDA_CHECK_RETURN(cudaMemsetAsync(particlesTilesOffset.data(), 0, numParticles * sizeof(uint32_t), cudaStream), logger);
            ...
            CUDA_CHECK_RETURN(cudaMemsetAsync(particlesProjectedPosition.data(), 0, numParticles * sizeof(vec2), cudaStream), logger);
            CHECK_STATUS_RETURN(particlesProjectedConicOpacity.resize(numParticles * sizeof(vec4), queueHandle, logger));
            CUDA_CHECK_RETURN(cudaMemsetAsync(particlesProjectedConicOpacity.data(), 0, numParticles * sizeof(vec4), cudaStream), logger);
            CHECK_STATUS_RETURN(particlesProjectedExtent.resize(numParticles * sizeof(vec2), queueHandle, logger));
            CUDA_CHECK_RETURN(cudaMemsetAsync(particlesProjectedExtent.data(), 0, numParticles * sizeof(vec2), cudaStream), logger);
            CHECK_STATUS_RETURN(particlesGlobalDepth.resize(numParticles * sizeof(float), queueHandle, logger));
            CUDA_CHECK_RETURN(cudaMemsetAsync(particlesGlobalDepth.data(), 0, numParticles * sizeof(float), cudaStream), logger);
        }
