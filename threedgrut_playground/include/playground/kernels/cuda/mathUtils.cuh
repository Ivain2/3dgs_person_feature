...
static __device__ inline float3 clampf3(const float3& a, const float _min, const float _max)
{
    return make_float3(clamp(a.x, _min, _max), clamp(a.y, _min, _max), clamp(a.z, _min, _max));
}
...
static __device__ inline float pi()
{
    return 3.141592654f;
}
